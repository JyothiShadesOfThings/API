...
module.exports.GetTissueDistribution_Peri = async (req,res)=>{

  
  

    const upload_id = req.query.upload_id; 
    const query = dbconnection.query(`select case when label ='HealedWound' 
    then 'Healed Wound' 
    ...
    group by  awcd.label
     ...
     from algo_peri_wound_details where upload_id=? group by upload_id) awd
     left join (select upload_id, sum(area) as area 
     from algo_wound_characterstics_details
